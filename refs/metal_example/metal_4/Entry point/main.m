...
Abstract:
Multiplies matrices on a GPU with the Metal API's machine learning features
 and verifies the product against a CPU implementation.
*/
...
static Matrix * _Nullable multiplyMatricesOnGPU(Matrix *matrix1,
    ...
    if (!success) {
        NSLog(@"Can't configure a matrix multiplier.");
        return nil;
    }

    [multiplier encodeAndRunModelInference];
    Matrix *product = [[Matrix alloc] initFromTensor:[multiplier productTensor]];
    return product;
}

/// Prints whether the two product matrices are equal.
/// - Parameters:
